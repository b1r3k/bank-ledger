...
2. Rename project:
 - Rename app folder to your app name if needed (`mv bank_ledger new_bank_ledger`) and then:

       find ./ -type f -not -path "./.git/*" -exec sed -i 's/bank_ledger/new_bank_ledger/g' {} \;
       find ./ -type f -not -path "./.git/*" -exec sed -i 's/bank_ledger/new-bank_ledger/g' {} \;
 - or use `make rename-project NEW_APP_NAME=new_bank_ledger` to do it for you
